 ...
 */
exports.basicAuth = function (username, password, username2, password2) {
  return function (req, res, next) {
    if (!(username && password) && !(username2 && password2)) {
    ...
    if (user && ((user.name === username && user.pass === password) || (user.name === username2 && user.pass === password2))) {
      next();
    } else {
  ...
  portScanner.findAPortNotInUse(port, port + 50, '127.0.0.1', function (error, availablePort) {
    if (port === availablePort) {
      ...
      prompt.get([{
        ...
        if (err) {
          console.error(err);
        } else if (result.answer.match(/y(es)?/i)) {
          // User answers yes
          port = availablePort;
